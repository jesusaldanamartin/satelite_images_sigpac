...
def index_values(lista_points,polygon, output):
    # index_points=[]
                ...
                if __name__ == '__main__':
                    ...
                    with Manager() as manager:
                        ...
                        for i in range(len(chunked_list)):
                            p = Process(target = function, kwargs={"lista_points":chunked_list[i], "polygon":g, "output":ind_values})
                            p.start()
                            process_list.append(p)
                        for p in process_list:
                            p.join()
